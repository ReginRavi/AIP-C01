...
## 48.1 Real-World Exam Scenario: Hybrid Patch Baseline Sync & Automated Patching (SSM Patch Manager + Maintenance Windows)

- **Scenario**: A company operates a hybrid cloud architecture connected via AWS Direct Connect LAG ports. They have an on-premises patch management system and need to synchronize patch baselines to all EC2 instances in an AWS VPC, as well as automate the patching schedule with the **LEAST amount of operational effort**. What is the optimal solution?
- **Architecture Solution**:
  - Use **AWS Systems Manager Patch Manager** to manage and deploy security patches to EC2 instances based on on-premises patch baselines.
  - Install the **SSM Agent** on all instances and automate the patching schedule using **AWS Systems Manager Maintenance Windows**.

```mermaid
flowchart TD
    subgraph OnPremises["1. On-Premises Data Center"]
        OnPremPatching["On-Premises Patch System<br/>(Defines OS Patch Baselines & Rules)"]
    end

    subgraph SystemsManager["2. AWS Systems Manager Governance Suite"]
        PatchManager["AWS Systems Manager Patch Manager<br/>• Configured with Custom Patch Baselines<br/>• Auto-approves security patches"]
        MaintWindow["SSM Maintenance Window<br/>⏰ Defines automated patch schedule<br/>(e.g., Every Sunday at 02:00 UTC)"]

        OnPremPatching ==>|"Sync Patch Rules"| PatchManager
        MaintWindow ==>|"Trigger Patch Task"| PatchManager
    end

    subgraph EC2Fleet["3. Target AWS EC2 Fleet (VPC)"]
        SSMAgent["SSM Agent<br/>(Installed on EC2 Instances)"]
        PatchExec["AWS-RunPatchBaseline Document<br/>⚡ Scans & applies approved OS patches"]

        PatchManager ==> SSMAgent ==> PatchExec
    end

    classDef onprem fill:#fff3cd,stroke:#ffc107,stroke-width:1px;
    classDef ssm fill:#d4edda,stroke:#28a745,stroke-width:2px;
    classDef ec2 fill:#7950f2,stroke:#5f3dc4,color:#ffffff;

    class OnPremPatching onprem;
    class PatchManager,MaintWindow ssm;
    class SSMAgent,PatchExec ec2;
```

### Key Technical Rationale:
1. **SSM Patch Manager for OS Patching**: **Patch Manager** automates scanning and patching of managed instances (Linux and Windows) against custom or AWS-defined **Patch Baselines** (specifying auto-approval rules and patch classifications).
2. **SSM Maintenance Windows for Automated Schedules**: **Maintenance Windows** allow defining a recurring schedule (e.g. cron schedule), duration, target instance tags, and automated tasks (invoking `AWS-RunPatchBaseline`) with zero custom code maintenance.
3. **SSM Agent Requirement**: Installed SSM Agents are mandatory for instances to register as managed nodes and execute Systems Manager tasks.

### Why Distractor Options Fail:
- *Session Manager for Patching*: **Session Manager** is designed for secure, audited interactive terminal access without SSH; it does NOT manage patch baselines or automate OS patching workflows.
- *AWS Lambda + Run Command for Scheduling*: Writing custom Lambda functions to trigger Run Command on a cron schedule introduces unnecessary custom code and maintenance compared to natively using **Maintenance Windows**.
- *State Manager + Distributor*: **State Manager** enforces desired configuration state (e.g. software installation anti-drift); **Distributor** packages custom software applications. Neither handles OS security patch baselines, which is the dedicated domain of **Patch Manager**.
