...
flowchart TD
    subgraph AntiPattern["❌ Single Account Anti-Pattern (High Risk)"]
        A1["Single AWS Account\n(Prod + Dev + Security + Logs + Sandbox)"]
    ...
    subgraph BestPractice["✅ Multi-Account Best Practice (Isolated Boundaries)"]
        ...
        OrgRoot --> SecOU["Security OU\n• Log Archive Account\n• Audit / Security Tooling"]
        OrgRoot --> WorkOU["Workloads OU\n• Production Account\n• Development Account"]
        OrgRoot --> SandOU["Sandbox OU\n• Isolated Experimentation"]
    end
...
flowchart TD
    subgraph AWS_Org["AWS Organization Structure"]
        ...
        subgraph CoreOU_Accounts["Security & Infrastructure"]
            LogAcc["Log Archive Account\n(Central S3 Bucket with MFA Delete & Object Lock)"]
            AuditAcc["Audit / Security Tooling Account\n(Security Hub, GuardDuty Delegated Admin)"]
            NetAcc["Network Account\n(Transit Gateway, Shared VPC via RAM)"]
        end
        ...
        subgraph SandboxOU_Accounts["Experimental"]
            SandAcc["Developer Sandbox Account\n(Aggressive SCP restrictions & Budget alerts)"]
        end
        
        CoreOU --> LogAcc & AuditAcc & NetAcc
        WorkloadsOU --> ProdOU & NonProdOU
        SandboxOU --> SandAcc
    end
...
flowchart TD
    subgraph Evaluation["Permission Evaluation Logic"]
        ...
        PB["IAM Permissions Boundary\n(Optional IAM User/Role Ceiling)"]
        IAM["Identity-Based IAM Policy\n(Explicit Allow Granted)"]
        
        Effective["Effective Working Permission\n(Intersection of all ALLOWs\nMINUS any Explicit DENYs)"]
        
        SCP & PB & IAM --> Effective
    end
...
flowchart LR
    subgraph Patterns["High-Yield Exam SCP Patterns"]
        P1["1. Region Restriction\nDeny all actions outside approved AWS Regions"]
        P2["2. Security Immutability\nDeny disabling CloudTrail, Config, or GuardDuty"]
        P3["3. Root User Lockdown\nDeny root account actions in member accounts"]
        P4["4. Network Guardrails\nDeny internet gateways (IGW) in private backend accounts"]
        P5["5. Encryption Enforcer\nDeny unencrypted EBS volume or S3 bucket creation"]
    end
...
flowchart TD
    subgraph CT_Components["AWS Control Tower Architecture"]
        ...
        CT --> LZ["1. Automated Landing Zone\n(Core Accounts + Security Baseline)"]
        CT --> AF["2. Account Factory\n(Vends pre-configured accounts via Service Catalog)"]
        CT --> GR["3. Controls / Guardrails\n(Preventive, Detective, Proactive)"]
        CT --> DB["4. Central Dashboard\n(Real-time compliance & drift detection)"]
    end
...
sequenceDiagram
    ...
    CT->>NewAcc: Create AWS Account & Assign to Workload OU
    CT->>NewAcc: Deploy Baseline VPC & AWS Config Rules
    ...
    CT->>Identity: Provision SSO Permission Sets & User Access
...
flowchart TD
    ...
    Q1 -- "Automated landing zone setup & account vending" --> PickCT["Use AWS Control Tower\n(Landing Zone + Account Factory + Guardrails)"]
    Q1 -- "Foundational account hierarchy & policy limits" --> PickOrg["Use AWS Organizations\n(OUs + SCPs + Consolidated Billing)"]
    Q1 -- "Central human workforce login & SSO" --> PickSSO["Use AWS IAM Identity Center\n(SAML 2.0 IdP Federation + Permission Sets)"]
    Q1 -- "Share network/compute resources cross-account" --> PickRAM["Use AWS RAM\n(Share Transit Gateway, Subnets, License Manager)"]
    Q1 -- "Central compliance & configuration tracking" --> PickConfig["Use AWS Config Aggregator\n(Multi-Account Multi-Region Data Aggregation)"]

    PickOrg --> Q2{"Need to enforce strict organizational restrictions?"}
    Q2 -- Yes --> UseSCP["Deploy Service Control Policies (SCPs)\n(Remember: Deny overrides all, does not apply to Mgmt Account)"]
