...
### Scenario 4: High-Concurrency Global Weather Forecast App (Amazon EFS + EC2 + CloudFront)
- **Scenario**: A weather forecasting app generates 20 GB of data (1 billion data points) every 15 minutes, overwriting older forecast data. Concurrent user traffic reaches 1,500 req/sec baseline, spiking up to 15,000 req/sec during severe weather events. Latency must remain under 2 seconds.
- **Solution**: Store weather forecast data points on an **Amazon EFS volume** mounted on an **Auto Scaling fleet of EC2 instances** behind an **ELB**, fronted by **Amazon CloudFront** with a 15-minute `Cache-Control` TTL.
- **Reasoning**:
  - **EFS vs S3**: Overwriting 1 billion individual objects on S3 every 15 minutes creates HTTP request bottlenecks. EFS provides parallel 10+ GB/s throughput and low operational latency for EC2.
  - **CloudFront TTL**: 15-minute edge caching matches the forecast cycle, absorbing 15,000 req/sec traffic spikes without swamping origin servers.
  - *Exam Traps*: Lambda default burst concurrency (500–3,000 req/sec) throttles under 15,000 req/sec spikes; Lambda@Edge is for request customization, not core dataset caching.
