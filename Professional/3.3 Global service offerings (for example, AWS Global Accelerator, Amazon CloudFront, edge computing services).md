...
flowchart TD
    ...
    subgraph AWS_Origins ["4. Regional Target Origins"]
        S3Bucket["Amazon S3 Bucket / ALB"]
        MultiRegionALB["ALB / NLB / EC2 (us-east-1 & eu-west-1)"]
        S3Target["Target Amazon S3 Bucket"]
    end
    ...
    EdgePoP -->|"HTTP/HTTPS Static & Dynamic Caching"| CloudFront
    EdgePoP -->|"Non-HTTP TCP/UDP & Instant Anycast Routing"| AGA
    EdgePoP -->|"Accelerated S3 File Uploads/Downloads"| S3TA
...
flowchart TD
    ...
    Q_Logic -- "Simple URL rewrites / Header manipulation / Redirects" --> CFF["CloudFront Functions<br/>- Lightweight JavaScript<br/>- Sub-millisecond startup<br/>- 210+ Edge Locations"]
...
flowchart TD
    ...
    subgraph EdgeLayer ["1. Amazon CloudFront Global Edge Locations"]
        ...
        BlockResponse["Return HTTP 403 Forbidden at Edge<br/>(Request Blocked BEFORE Reaching Origin!)"]
        CacheCheck{"3. In Edge Cache?"}
        ServeCache["Return Low-Latency Cached File"]
        FetchOrigin["4. Fetch from Origin & Cache"]
    end
    ...
    EdgeCheck -- "2. Geo-IP Matches Blacklist" --> BlockResponse

    ApprovedUser ==>|"1. Request Report File"| EdgeCheck
    EdgeCheck -- "2. Geo-IP Approved" --> CacheCheck
    CacheCheck -- "YES" --> ServeCache
    CacheCheck -- "NO" --> FetchOrigin
    FetchOrigin ==> OriginApp

    classDef danger fill:#f8d7da,stroke:#dc3545,color:#000000;
    classDef safe fill:#d4edda,stroke:#28a745,color:#000000;
...
flowchart TD
    ...
    subgraph OriginGroupLayer ["2. CloudFront Origin Failover Group"]
        ...
        CF_Failover["CloudFront Automatic Origin Failover Engine"]
    ...
    PrimaryOrigin -.->|"2. Returns HTTP 504 Error"| CF_Failover
    CF_Failover ==>|"3. Automatic Failover Switch"| SecondaryOrigin

    classDef edge fill:#fff3cd,stroke:#ffc107,color:#000000;
    classDef primary fill:#f8d7da,stroke:#dc3545,color:#000000;
    classDef secondary fill:#d4edda,stroke:#28a745,color:#000000;
...
flowchart TD
    ...
    subgraph DNS_Routing ["1. Amazon Route 53 Public DNS"]
        R53_Apex["Route 53 Public Hosted Zone<br/>Alias A Record: example.com to Global Accelerator DNS"]
    end
...
flowchart TD
    ...
    CF_Edge ==>|"Encrypted POST Data via HTTPS"| ALB
    ALB ==> EC2_App
    EC2_App --> PrivKey
    PrivKey --> EC2_App
    EC2_App ==> OriginHeader
    OriginHeader ==> CF_Edge
...
flowchart TD
    ...
    Mobile ==>|"HTTP GET"| CF_Dist
    Desktop ==>|"HTTP GET"| CF_Dist
    CF_Dist ==> LambdaEdge
...
flowchart TD
    ...
    CF --> RecentBehavior
    CF --> ArchiveBehavior
    RecentBehavior ==>|"Restricted OAC Read Access"| S3Bucket
    ArchiveBehavior ==>|"Restricted OAC Read Access"| S3Bucket
...
flowchart TD
    ...
    subgraph FactoryEdge ["2. On-Premises Factory Data Center (No Internet Required)"]
        ...
        subgraph GreengrassServer ["Local Edge Compute Server"]
            GG_Core["AWS IoT Greengrass Core Software<br/>Executes Local ML Inference Offline (Zero Internet Dependency)"]
            GG_Comp["Greengrass Custom Component<br/>Integrates directly with Local Linux Server API"]
        end

        LinuxAPI["On-Premises Linux Server API<br/>Receives Real-Time Defect Feedback for Assembly Workers"]
    end
...
flowchart TD
    ...
    subgraph Solution1 ["Option 1: CloudFront Signed URL + OAC"]
        ...
        OAC["Origin Access Control (OAC)<br/>Restricts Bucket Access to CloudFront ONLY"]
    ...
    User ==>|"Access via Signed URL"| CF_URL
    CF_URL ==> CF_Edge
    CF_Edge <--> OAC
    OAC ==> S3_Private1

    User ==>|"Access via S3 Presigned URL"| S3_Presigned
    S3_Presigned ==> S3_Private2
...
flowchart TD
    subgraph Viewers ["1. Public Viewers & End Users"]
        User["Website Visitors<br/>(Route 53 Alias to CloudFront)"]
    end

    subgraph CloudFrontEdge ["2. Amazon CloudFront Distribution Edge Tier"]
        CF["CloudFront Distribution<br/>Route /static/* to S3<br/>Route /api/* to ALB"]
    ...
    subgraph StaticOrigin ["3. Static Content Storage Origin"]
        OAC["Origin Access Control (OAC)<br/>SigV4 Authentication Header"]
    ...
    subgraph DynamicOrigin ["4. Dynamic Compute & Load Balancer Origin"]
        WAF_ALB["AWS WAF Web ACL (Associated with ALB)<br/>Web Rule: Deny Requests Without X-Origin-Verify"]
        ALB["Application Load Balancer (ALB)"]
        Fargate["AWS Fargate Cluster Tasks"]
    end

    User ==>|"HTTPS Request"| CF
    CF ==>|"/static/* (OAC Signed)"| OAC
    OAC ==> S3Bucket

    CF ==>|"/api/* (Injects Custom Header)"| CustomHeader
    CustomHeader ==> WAF_ALB
    WAF_ALB ==> ALB
    ALB ==> Fargate
...
flowchart TD
    ...
    subgraph CloudFrontTier ["2. Amazon CloudFront Edge Tier"]
        ...
        EdgeCache{"CloudFront Edge Cache Lookup"}
        CacheHit["CACHE HIT!<br/>(Served instantly from Edge Location<br/>Zero Load on On-Premises Servers)"]
    end

    subgraph OnPremOrigin ["3. On-Premises Data Center Tier (Custom Origin)"]
        OnPremELB["On-Premises Load Balancer<br/>Configured as CloudFront Custom Origin"]
    ...
    EdgeCache -- "Cache Miss / Dynamic Request" --> OnPremELB
    OnPremELB ==> WebFleet
    WebFleet ==> OracleDB
...
flowchart TD
    ...
    CloudFront ==>|"2. Encrypted HTTPS Origin Request (Port 443)"| ALB
    ALB ==> EC2Fleet
