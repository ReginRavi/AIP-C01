...
> [!IMPORTANT]
> **Key Exam Rule**: SCPs apply to all member accounts in an organization or OU, but **NEVER apply to the Management Account**. This is why workloads must never run in the Management Account.
...
> 3. **RAM vs. VPC Peering**: When connecting dozens of VPCs across different accounts, VPC Peering creates an unmanageable mesh. Use **AWS RAM + AWS Transit Gateway**.
> 4. **Over-Engineering Control Tower**: If a question only asks to aggregate billing and set up account permission boundaries, **AWS Organizations + SCPs** is the minimal solution. Do not select Control Tower unless automated landing zones or managed guardrails are requested.
