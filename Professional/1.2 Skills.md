...
## 48.1 Real-World Exam Scenario: Generating Least-Privilege IAM Policies (CloudTrail + Access Analyzer)

- **Scenario**: During an audit, a cloud governance team finds that Lambda execution roles have overly broad permissions (e.g., full access to S3 and DynamoDB). The team must restrict permissions to the absolute minimum required by each function (**principle of least privilege**) with the **LEAST operational effort**.
- **Correct 2-Step Solution**:
  1. **Create a trail in AWS CloudTrail** and enable CloudTrail logging to record S3 and DynamoDB data events.
  2. **Use IAM Access Analyzer** to review the CloudTrail logs and automatically **generate a granular IAM policy template** based on the actual recorded API activity.
- **Reasoning & Exam Heuristic**:
  - IAM Access Analyzer includes a built-in **Policy Generation** feature that analyzes CloudTrail logs over a specified date range for a specific user or execution role and outputs a ready-to-use least-privilege IAM policy.
  - *Why Incorrect Options Fail*:
    - **Amazon GuardDuty**: Threat detection service; does NOT generate IAM policies.
    - **AWS Audit Manager**: Compliance audit & framework evidence collector; does NOT analyze raw API calls to create fine-grained IAM policies.
    - **AWS CodeGuru Profiler**: Analyzes application CPU/memory performance; does NOT inspect IAM API calls.
