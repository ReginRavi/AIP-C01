...
### Scenario 5: High-Concurrency Global Weather Forecast App (Amazon EFS + EC2 + CloudFront)
- **Scenario**: A global mobile weather forecasting application generates 20 GB of weather forecast data (1 billion data points, 20 bytes each) every 15 minutes, with each update overwriting older forecast data. Concurrent user queries reach 1,500 req/sec baseline and spike 10x up to 15,000 req/sec during severe weather events. Queries must complete in under 2 seconds globally.
- **Solution**: Store forecast data points on an **Amazon EFS volume** mounted across an **Auto Scaling fleet of Amazon EC2 instances** behind an **Elastic Load Balancer (ELB)**, fronted by an **Amazon CloudFront distribution** with a **15-minute `Cache-Control` TTL**.
- **Reasoning**:
  - **Amazon EFS vs. Amazon S3**: Writing 1 billion individual objects every 15 minutes to S3 introduces severe HTTP request overhead and latency. Amazon EFS provides parallel unconstrained NFS storage (10+ GB/sec throughput) and low latency across EC2 instances.
  - **Amazon CloudFront Edge Caching**: A 15-minute TTL matches the forecast refresh rate. Edge locations serve global traffic spikes (15,000 req/sec) with sub-second latency without swamping origin EC2/EFS capacity.
  - *Exam Traps*:
    - **AWS Lambda + OpenSearch**: Lambda default burst concurrency (500–3,000 req/sec) causes heavy throttling during 15,000 req/sec traffic spikes.
    - **Lambda@Edge for Data Caching**: Lambda@Edge customizes request/response headers and routing at edge locations; it is not a core dataset cache.
    - **S3 Bucket Writes**: Overwriting 1 billion individual S3 objects every 15 minutes takes significantly longer than updating files on an Amazon EFS filesystem.
