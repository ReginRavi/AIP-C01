...
### Scenario 8.5: Rapid OS Vulnerability Mitigation & Patch Compliance Auditing (SSM Patch Manager + AWS Config vs. Weekly AMI, State Manager & Control Tower Traps)

- **Scenario**: A security vulnerability was discovered in the outdated operating system of an EC2 instance fleet hosted inside a VPC. The Solutions Architect must mitigate the vulnerability as soon as possible by deploying OS security patches AND continuously record all changes to patches and association compliance statuses over time. What is the recommended solution?
- **Architecture Solution**:
  - Use **AWS Systems Manager Patch Manager** to deploy OS security patches on the EC2 instances.
  - Use **AWS Config** to manage, detect, and record the security patch association compliance status of the EC2 instances over time.

```mermaid
flowchart TD
    subgraph VulnerableFleet ["1. Amazon EC2 Compute Fleet (VPC)"]
        EC2_Fleet["EC2 Instance Fleet<br/>(Outdated OS Vulnerability)"]
    end

    subgraph PatchingEngine ["2. Automated Vulnerability Mitigation Engine"]
        PatchBaseline["SSM Patch Baseline<br/>📜 Auto-approves Security Updates"]
        PatchManager["AWS Systems Manager Patch Manager<br/>⚡ Scans & Installs Missing Security Patches"]

        PatchBaseline ==> PatchManager ==>|"Deploys Security Patches"| EC2_Fleet
    end

    subgraph ComplianceRecording ["3. Continuous Configuration & Compliance Recording"]
        AWSConfig["AWS Config<br/>📊 Continuously Monitors, Records & Audits Resource Configurations"]
        ComplianceTimeline["Patch & Association Compliance History Timeline"]

        EC2_Fleet ==>|"Reports Association & Patch Compliance Status"| AWSConfig ==> ComplianceTimeline
    end
    ...
    class EC2_Fleet ec2;
    class PatchBaseline,PatchManager patch;
    class AWSConfig,ComplianceTimeline config;
```

### Key Technical Rationale:
1. **AWS Systems Manager Patch Manager for Immediate Vulnerability Mitigation**:
   - Patch Manager automates the process of scanning and applying OS security updates across Linux and Windows EC2 instance fleets without manual intervention or custom image baking delays.
2. **AWS Config for Patch Association & Compliance Tracking**:
   - AWS Config continuously monitors, records, and audits resource configuration changes and SSM association compliance statuses over time, maintaining a complete historical timeline of patch compliance for security audits.

### Why Distractor Options Fail:
- *Weekly Golden AMI Baking & Rollout*:
  - **Lacks Patch Control & Compliance Recording**: Baking a new AMI weekly lacks granular control over specific security patches, complicates rollback operations if an update breaks application logic, and fails to record individual patch association compliance history in a dedicated auditing engine.
- *SSM State Manager + Amazon OpenSearch*:
  - **Misaligned Engine & Excessive Overhead**: SSM State Manager maintains static node configuration states (e.g. software installs, script execution); it is not purpose-built for OS security patch baselines. Streaming patch logs to Amazon OpenSearch adds unnecessary infrastructure cost and operational overhead compared to native **AWS Config**.
- *AWS Control Tower + Amazon Managed Grafana*:
  - **Service Misuse**: AWS Control Tower automates multi-account AWS Organizations landing zone setup—it CANNOT deploy OS security patches to EC2 instances! Amazon Managed Grafana is a data visualization dashboard, not a configuration compliance recorder.

---

## 9. Final Mental Model

`Tag (Environment + OS) ➔ Baseline (SSM Patch Baseline for Security Patches) ➔ Audit (AWS Config Managed Rules & Compliance Recording) ➔ Alert (CloudWatch / EventBridge)`