...
### Section 103.2: Short-Lived One-Time EMR Batch Cluster Purchasing Strategy (On-Demand Master/Core + Spot Task Nodes vs. Reserved Instances & Spot Master Traps)

- **Scenario**: A scientific research organization (CERN) runs a one-time 8-hour batch data processing job ingesting 300 TB of particle accelerator data using an Amazon EMR cluster of 200 EC2 instances. The cluster will only be run once, and the resulting dataset will be temporarily stored before loading into Amazon Redshift. What is the best and most cost-effective purchasing option across the EMR node types?
- **Architecture Solution**:
  - Use **On-Demand EC2 instances** for both the **Master node** and **Core nodes**.
  - Use **Spot EC2 instances** for the **Task nodes**.

```mermaid
flowchart TD
    subgraph EMR_Cluster ["Amazon EMR Cluster (200 EC2 Instances / One-Time 8-Hour Run)"]
        subgraph MasterTier ["1. Master Node (Cluster Management & Metadata)"]
            Master["Master Node<br/>(1 Instance)"]
            MasterOD["On-Demand EC2 Instance<br/>⚠️ Prevent Interruption!<br/>(Spot interruption = Entire cluster terminates)"]
            Master ==> MasterOD
        end

        subgraph CoreTier ["2. Core Nodes (HDFS Storage + Processing)"]
            Core["Core Nodes<br/>(HDFS Data Storage)"]
            CoreOD["On-Demand EC2 Instances<br/>🛡️ Protect HDFS Data!<br/>(Spot interruption = Lost HDFS blocks & data corruption)"]
            Core ==> CoreOD
        end

        subgraph TaskTier ["3. Task Nodes (Stateless Compute Only)"]
            Task["Task Nodes<br/>(Pure Compute Workers)"]
            TaskSpot["Spot EC2 Instances<br/>💰 Max 90% Cost Savings!<br/>(Stateless compute; tasks rescheduled on interruption)"]
            Task ==> TaskSpot
        end
    end

    classDef master fill:#f8d7da,stroke:#dc3545,stroke-width:1px;
    classDef core fill:#d1ecf1,stroke:#17a2b8,stroke-width:1px;
    classDef task fill:#2b8a3e,stroke:#1e632b,color:#ffffff;

    class Master,MasterOD master;
    class Core,CoreOD core;
    class Task,TaskSpot task;
```

### Key Technical Rationale:
1. **On-Demand Master Node**: The Master node coordinates the entire Hadoop/Spark cluster. If the Master node is interrupted, the entire EMR cluster terminates instantly. Therefore, Master nodes must run on On-Demand instances (or Spot Blocks if supported) to prevent total cluster failure.
2. **On-Demand Core Nodes**: Core nodes run tasks AND host the Hadoop Distributed File System (HDFS). If a Core node is interrupted via a Spot reclamation, HDFS data blocks stored on that node are lost, risking data corruption or triggering expensive HDFS re-replication network overhead across the 300 TB dataset.
3. **Spot Task Nodes**: Task nodes execute pure, stateless compute processing and do NOT host HDFS storage. If a Spot Task node is reclaimed by AWS, EMR simply reschedules the stateless task onto another worker node without data loss, offering up to 90% cost savings for the bulk of the 200-node cluster.
4. **Why Reserved Instances Fail**: Reserved Instances require purchasing a **1-year or 3-year contract commitment** for 24/7 continuous workloads. Purchasing 1-year or 3-year Reserved Instances for a single, transient 8-hour batch job is massively wasteful and cost-prohibitive.

### Why Distractor Options Fail:
- *Using Reserved EC2 Instances for Master and Core Nodes (Your Selection)*:
  - **Contract Commitment Misunderstanding**: Reserved Instances require 1-year or 3-year commitments. They are completely unsuited for a short-lived 8-hour batch execution.
- *Using Spot Instances for Master and Core Nodes*:
  - **Data Loss & Cluster Termination**: Running Master on Spot risks instant cluster termination; running Core nodes on Spot risks HDFS data block loss and job corruption.
- *Using Reserved Instances for Master + On-Demand Core Nodes*:
  - **Wasted 1-Year/3-Year Commitment**: Still incurs 1-year or 3-year contract commitments for the Master node for a one-time 8-hour run.
...
## 21. Real-World Exam Scenario: Dynamic Public Transit Event Spikes & Hybrid Compute Purchasing (Reserved Instances + On-Demand & Spot ASG vs. Pure Spot & Static Allocation Traps)

- **Scenario**: A travel booking company hosts its public transit trip planner application on AWS (ALB + Auto Scaling Group of EC2 instances across 3 Availability Zones + Multi-AZ RDS). During major concerts and sporting events in a city, traffic spikes cause EC2 utilization to exceed 90%. The company requires the most cost-effective architectural design to maintain high availability and rapidly recover if an Availability Zone fails during peak usage. What solution should be implemented?
- **Architecture Solution**:
  - Increase the capacity and scaling thresholds of the **Auto Scaling Group** to allow application servers to scale out dynamically across all 3 Availability Zones.
  - Use **Reserved Instances (or Savings Plans)** to cover the steady-state baseline load.
  - Use a combination of **On-Demand and Spot Instances** within the Auto Scaling Group to process the peak event load.
  - Scale down the On-Demand and Spot instances automatically when peak usage subsides.

```mermaid
flowchart TD
    subgraph TrafficProfile ["1. Incoming Application Traffic Profile"]
        Baseline["Steady-State Baseline Traffic<br/>(24/7/365 regular transit searches)"]
        PeakSurge["Event Traffic Spikes (>90% CPU)<br/>(Sporting events & popular concerts)"]
    end

    subgraph ComputePurchasing ["2. Hybrid Compute Purchasing & Scaling Strategy"]
        RI["Reserved Instances / Savings Plans<br/>💰 40-72% Discount for Steady-State"]
        ASG["Auto Scaling Group across 3 AZs<br/>⚡ Dynamic Multi-AZ Scaling"]
        SpotOD["Mixed Instances Policy<br/>🏷️ Spot Instances (up to 90% off) + On-Demand Fallback"]

        Baseline ==> RI
        PeakSurge ==> ASG ==> SpotOD
    end

    subgraph AutoScaleBehavior ["3. Event Peak Scaling Cycle"]
        ScaleOut["Peak Event Starts ➔ ASG Scales Out On-Demand & Spot Fleet"]
        ScaleIn["Peak Event Ends ➔ ASG Scales In & Terminates Excess Capacity"]

        SpotOD ==> ScaleOut ==> ScaleIn
    end

    classDef traffic fill:#fff3cd,stroke:#ffc107,stroke-width:1px;
    classDef compute fill:#7950f2,stroke:#5f3dc4,color:#ffffff;
    classDef autoscale fill:#2b8a3e,stroke:#1e632b,color:#ffffff;

    class Baseline,PeakSurge traffic;
    class RI,ASG,SpotOD compute;
    class ScaleOut,ScaleIn autoscale;
```

### Key Technical Rationale:
1. **Hybrid Purchasing Mix Strategy**:
   - **Reserved Instances (or Savings Plans)**: Best for steady-state baseline workloads running 24/7/365, delivering maximum discount (40–72%).
   - **Spot Instances + On-Demand Instances (via ASG Mixed Instances Policy)**: Best for handling short-term, spiky event traffic (concerts/games). Spot instances reduce peak compute costs by up to 90%, while On-Demand acts as a fallback if Spot capacity is unavailable.
2. **Dynamic Multi-AZ Auto Scaling**:
   - Configuring the Auto Scaling Group across 3 Availability Zones ensures that if a single AZ experiences an outage during a major event, the ASG automatically launches replacement On-Demand/Spot instances in the remaining 2 healthy AZs to absorb the load.

### Why Distractor Options Fail:
- *Replace ALL Instances with Spot Instances in a Single AZ*:
  - **Catastrophic Outage Risk**: Spot instances can be reclaimed by AWS with a 2-minute notice. Running baseline steady-state traffic exclusively on Spot in a single AZ risks a total application outage if Spot capacity is revoked or if that AZ fails.
- *Deploy Fixed 6 Reserved + Spot Instances per AZ without Auto Scaling*:
  - **High Static Overhead**: Maintaining static over-provisioned instances 24/7 incurs massive unnecessary cost during normal non-event hours and fails to leverage dynamic elasticity.
- *Deploy Fixed 1 On-Demand + 2 Spot Instances per AZ without Auto Scaling*:
  - **No Dynamic Scaling or Failover Protection**: Static instance allocations without an Auto Scaling Group cannot scale out during massive concert traffic spikes or automatically replace failed instances when an AZ goes down.

---

## 21.1 Real-World Exam Scenario: High Availability & Cost-Effective AZ Recovery (Spot Fleet with On-Demand + Spot Mix)

- **Scenario**: A digital media publishing company operates an online portal on a single EC2 instance currently operating at 90% peak utilization. The company wants to enable the portal to **quickly recover in the event of an Availability Zone (AZ) outage** using the **MOST cost-effective solution**.
...
- **Key Architectural Rationale**:
  - **Mixed Capacity Strategy (On-Demand + Spot)**:
    - **On-Demand Instances**: Handle core steady-state baseline traffic, ensuring guaranteed compute availability without risk of reclamation.
    - **Spot Instances**: Scale to handle peak usage spikes at up to a 90% discount compared to On-Demand pricing.
  - **Multi-AZ Spot Fleet Distribution + ALB**:
    - Distributing the Spot Fleet across multiple AZs behind an ALB guarantees that if an AZ fails, the Spot Fleet automatically maintains target capacity by launching replacement instances in healthy AZs.
    - **Spot Pool Diversification**: Configuring a Spot Fleet across multiple instance types (`c5.large`, `m5.large`, `r5.large`) ensures that if one instance type is reclaimed by AWS, other instance types remain running.
  - *Why Incorrect Options Fail*:
    - **100% On-Demand Auto Scaling Group**: Provides high availability, but is significantly more expensive because it ignores Spot instance discounts for peak load.
    - **100% Spot Fleet (or 100% Spot ASG)**: Highly vulnerable to service disruption. If AWS reclaims Spot capacity due to market price increases, the baseline application crashes.
