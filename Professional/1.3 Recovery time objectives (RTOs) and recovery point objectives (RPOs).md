...
flowchart LR
    subgraph DataLoss["RPO (Recovery Point Objective)"]
        ...
        P1 -.- |"Maximum Acceptable Data Loss<br/>(Measured in Time: Sec / Min / Hours)"| P2
    end

    subgraph Downtime["RTO (Recovery Time Objective)"]
        direction TB
        T1["Disaster Strikes (T = 0)"] --> T2["Application Fully Restored"]
        T1 -.- |"Maximum Acceptable Downtime<br/>(Measured in Time: Sec / Min / Hours)"| T2
    end
...
```mermaid
flowchart TD
    BizReq["Business Impact Analysis<br/>(Cost of Downtime vs. Cost of Prevention)"] --> SetTargets["Define Strict RTO & RPO Targets"]
    
    SetTargets --> RPO_Path["RPO Target Determines:"]
    SetTargets --> RTO_Path["RTO Target Determines:"]
    
    RPO_Path --> DataArch["Data Layer Architecture:<br/>• Backup Frequency<br/>• Synchronous vs. Asynchronous Replication<br/>• Cross-Region Storage"]
    RTO_Path --> ComputeArch["Compute & Routing Architecture:<br/>• Cold Standby vs. Warm vs. Active-Active<br/>• Automated vs. Manual Failover<br/>• DNS TTL & Health Checks"]

    DataArch --> CostOverhead["Final Solution Cost &amp;<br/>Operational Overhead"]
    ComputeArch --> CostOverhead
...
flowchart LR
    subgraph Progression["RTO / RPO vs. Cost Progression"]
        direction LR
        BR["1. Backup & Restore<br/>RTO: Hours-Days<br/>RPO: Hours-Days<br/>Cost: $"] --> PL["2. Pilot Light<br/>RTO: 15m - 2h<br/>RPO: Sec - Min<br/>Cost: $$"]
        PL --> WS["3. Warm Standby<br/>RTO: Minutes<br/>RPO: Sec - Sub-Min<br/>Cost: $$$"]
        WS --> AA["4. Multi-Site Active/Active<br/>RTO: Near-Zero (Sec)<br/>RPO: Near-Zero (Real-time)<br/>Cost: $$$$"]
    end
...
flowchart TD
    ...
    RPO_Target -- "Near-Zero (&lt; 1 Second)" --> SubSec["Continuous Multi-Master / Storage-Level Replication<br/>• DynamoDB Global Tables (Two-way multi-active)<br/>• Aurora Global Database (&lt; 1 sec replication lag)<br/>• S3 Multi-Region Access Points (MRAP) + RTC"]
    
    RPO_Target -- "Seconds to Minutes" --> SecMin["Asynchronous Database Replication & Block Sync<br/>• RDS Cross-Region Read Replicas<br/>• AWS Elastic Disaster Recovery (DRS continuous sync)<br/>• S3 Cross-Region Replication (CRR)"]
    
    RPO_Target -- "Hours to Days" --> HoursDays["Snapshot & Scheduled Backup Strategies<br/>• AWS Backup (Automated cross-region/cross-account)<br/>• Amazon EBS Snapshots / RDS Automated Backups<br/>• S3 Versioning & Lifecycle Glacier Archives"]
...
sequenceDiagram
    ...
    Primary --x Primary: Outage Occurs
    ...
    rect rgb(30, 50, 80)
        ...
        DR_Compute ->> DR_Compute: Auto Scaling scales up (0 to N or 10 to N)
...
flowchart TD
    ...
    Q1 -- "Hours / Days" --> BR["Backup & Restore<br/>(Lowest Cost, Low Ongoing Overhead)"]
    
    Q1 -- "Tens of Minutes to ~1-2 Hours" --> Q2{"Is data loss required to be minimal (seconds)?"}
    Q2 -- Yes --> PL["Pilot Light<br/>(Live DB sync, 0 compute)"]
    Q2 -- No --> BR

    Q1 -- "Minutes (&lt; 15 mins)" --> WS["Warm Standby<br/>(Live DB sync, small running compute fleet)"]
    
    Q1 -- "Seconds / Near-Zero" --> AA["Multi-Site Active/Active<br/>(Highest Cost, Highest Overhead)"]
