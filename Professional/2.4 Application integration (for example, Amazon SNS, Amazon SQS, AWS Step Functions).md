...
`Event Occurs ➔ Route (EventBridge) ➔ Fan-Out (SNS) ➔ Buffer (SQS + S3 CRR Storage) ➔ Real-Time Stream (Kinesis Data Streams + KCL) ➔ Mobile Push (SNS Mobile Push) ➔ Direct File Event (S3 Event Notifications to Lambda) ➔ Orchestrate (Step Functions)`