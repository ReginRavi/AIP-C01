...
# 45. Real-World Exam Scenario: Intermittent 50% Outbound REST API Failure (EC2 NAT Instance Failure vs. Managed NAT Gateway)

- **Scenario**: A multi-tier web application runs across 2 Availability Zones (AZs) in a VPC. Application Load Balancers operate in public subnets, and EC2 Auto Scaling instances operate in private subnets. Users report that the web application logins work, but a "find a nearby store" function that invokes a 3rd-party REST API map provider loads **only ~50% of the time upon page refresh**. Outbound API calls route through self-managed EC2 NAT instances in the public subnets. What is the MOST likely cause and recommended architectural solution?
- **Architecture Solution**:
  - **Root Cause**: One of the two self-managed EC2 NAT instances in a public subnet has **failed / crashed**. Traffic routed through the healthy AZ NAT instance succeeds (50%), while traffic routed through the failed AZ NAT instance fails (50%).
  - **Recommended Fix**: Replace self-managed EC2 NAT instances with fully managed **AWS NAT Gateways** in each public subnet across Availability Zones.

```mermaid
flowchart TD
    subgraph PrivateSubnets["1. Multi-AZ Private Subnets (App Fleet)"]
        EC2_AZa["EC2 Instance (AZ-a)"]
        EC2_AZb["EC2 Instance (AZ-b)"]
    end

    subgraph LegacyNATInstance["2. Faulty Legacy EC2 NAT Instances"]
        NAT_AZa["EC2 NAT Instance (AZ-a)<br/>✅ Healthy"]
        NAT_AZb["EC2 NAT Instance (AZ-b)<br/>💥 CRASHED / FAILED!"]

        EC2_AZa ==>|"0.0.0.0/0 Route"| NAT_AZa
        EC2_AZb ==>|"0.0.0.0/0 Route"| NAT_AZb
    end

    subgraph ExternalProvider["3. 3rd-Party REST API Map Provider"]
        API_Pass["50% Success (AZ-a Route)"]
        API_Fail["50% Failure (AZ-b Route)"]

        NAT_AZa ==> API_Pass
        NAT_AZb -.->|"Traffic Dropped"| API_Fail
    end

    subgraph ManagedFix["4. Recommended Fix: AWS NAT Gateways"]
        NGW_AZa["AWS NAT Gateway (AZ-a)<br/>• Fully Managed & Auto-Scaled"]
        NGW_AZb["AWS NAT Gateway (AZ-b)<br/>• Built-in AZ Redundancy"]
    end

    style NAT_AZa fill:#2b8a3e,stroke:#1e632b,color:#ffffff;
    style NAT_AZb fill:#c92a2a,stroke:#861b1b,color:#ffffff;
    style ManagedFix fill:#d4edda,stroke:#28a745,stroke-width:2px;
```

### Key Technical Rationale:
1. **Multi-AZ Route Distribution (50% Correlation)**: In a 2-AZ VPC, half the private subnet traffic routes through the AZ-a NAT device, and half routes through the AZ-b NAT device. If a self-managed EC2 NAT instance crashes, exactly 50% of outbound internet traffic fails upon page refresh.
2. **EC2 NAT Instance Single Point of Failure (SPOF)**: EC2 NAT instances are self-managed single instances. If they crash, AWS does not automatically recover or fail them over.
3. **AWS Managed NAT Gateway Benefits**: Managed NAT Gateways scale automatically from 5 Gbps up to 100 Gbps, provide built-in redundancy within each AZ, and eliminate self-managed EC2 NAT maintenance overhead.

### Why Distractor Options Fail:
- *Misconfigured Network ACL*: A misconfigured NACL blocking outbound traffic would cause **100% of map requests to fail**, not intermittent 50% failures upon refresh.
- *Third-party provider AZ failure*: External 3rd-party API endpoints failover globally; local 50% VPC failures correlate directly to local multi-AZ NAT infrastructure outages.
- *Overloaded NAT instance (Scale EC2 size)*: An overloaded NAT instance causes latency slowdowns or intermittent packet drops during peak traffic hours (not a deterministic 50% failure rate regardless of time/refresh). Upscaling EC2 NAT instances retains self-managed SPOF infrastructure risks compared to managed NAT Gateways.
