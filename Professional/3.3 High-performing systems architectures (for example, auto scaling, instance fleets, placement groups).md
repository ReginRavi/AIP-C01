...
flowchart TD
    subgraph Edge_Tier ["1. Global Edge & CDN Tier"]
        ...
        CF["Amazon CloudFront (Edge Caching & Latency Reduction)"]
    end
    ...
    subgraph Compute_Tier ["3. Auto Scaling Compute Tier (Multi-AZ)"]
        ASG["EC2 Auto Scaling Group (Target Tracking / Warm Pools)"]
        WarmPool["ASG Warm Pool (Pre-initialized Standby EC2s)"]
    end

    subgraph Caching_Database ["4. Database & Caching Tier"]
        ElastiCache["Amazon ElastiCache (Redis / Memcached Session Store)"]
        Aurora["Amazon Aurora Primary (Writes)"]
        Replicas["Amazon Aurora Read Replicas (Read Scaling)"]
    end

    R53 --> CF
    CF ==> ALB
    ALB ==> ASG
    WarmPool -.->|"Rapid Scale Out"| ASG
    ASG ==> ElastiCache
    ASG ==> Aurora
    Aurora ==> Replicas
...
flowchart TD
    subgraph Cluster_PG ["1. Cluster Placement Group (Low Latency / High Throughput)"]
        ...
        EC2_C1["EC2 Instance 1"]
        EC2_C2["EC2 Instance 2"]
        EC2_C3["EC2 Instance 3"]
    end

    subgraph Spread_PG ["2. Spread Placement Group (Hardware Fault Isolation)"]
        HW1["Hardware Rack 1"]
        EC2_S1["EC2 Instance A"]
        HW2["Hardware Rack 2"]
        EC2_S2["EC2 Instance B"]
        HW3["Hardware Rack 3"]
        EC2_S3["EC2 Instance C"]
    end

    subgraph Partition_PG ["3. Partition Placement Group (Large Distributed Clusters)"]
        Part1["Partition 1 (Hardware Set A)"]
        EC2_P1["EC2 Fleet 1..N"]
        Part2["Partition 2 (Hardware Set B)"]
        EC2_P2["EC2 Fleet 1..N"]
    end
...
flowchart TD
    ...
    subgraph DatabaseTier ["4. High-Availability & Read-Scaled Database Tier"]
        ...
        RDS_Standby[("Amazon RDS Multi-AZ Standby<br/>Passive Synchronous Replica for HA")]
...
flowchart TD
    ...
    subgraph ALB_Tier ["2. Application Load Balancer Nodes"]
        ALB_AZ1["ALB Node (AZ-1 Subnet Associated) - Enabled"]
        ALB_AZ2["ALB Node (AZ-2 Subnet Associated) - Enabled"]
        ALB_AZ3["AZ-3 Subnet NOT Associated with ALB - Disabled"]
    end
...
flowchart TD
    subgraph RegionalQuota ["Account-Level Regional Concurrency Quota (e.g. 1,000 Executions)"]
        ...
        subgraph UnreservedPool ["2. Unreserved Concurrency Pool (Protected Capacity)"]
            ...
            UnreservedQuota["Remaining 950 Concurrency Slots<br/>Protected from S3 Replication Bursts!"]
        end
    end
...
flowchart TD
    ...
    Node1 <-->|"OS Bypass Low-Latency MPI Interconnect"| Node2
    Node2 <-->|"OS Bypass Low-Latency MPI Interconnect"| NodeN

    Node1 ==>|"POSIX Parallel File System Read/Write"| FSxLustre
    Node2 ==>|"POSIX Parallel File System Read/Write"| FSxLustre
    NodeN ==>|"POSIX Parallel File System Read/Write"| FSxLustre
...
flowchart TD
    subgraph ImpairedFleet ["1. Impaired EC2 Instance Fleet"]
        ImpairedEC2["Impaired / Unreachable EC2 Instance<br/>Network Misconfiguration / RDP / OS Firewall Error"]
    end
    ...
    subgraph SSMAutomation ["2. Managed Systems Manager Automation Suite"]
        ...
        EC2RescueTool["EC2Rescue Tool<br/>Fixes OS Registry, Network Drivers, & Firewall Rules"]
    end
